...
function App() {
 ...
 const [formData, setFormData] = useState({
  ...
  const [m_jumin, setM_jumin] = useState(''); // 추가
  ...
  const handleChange = (e) => {
    ...
    if (name === 'm_jumin1') {
      ...
      setM_jumin(formattedValue + formData.m_jumin2); // 업데이트
      ...
      setM_jumin(formData.m_jumin1 + formattedValue); // 업데이트
    } else {
      setFormData((prevFormData) => ({
        ...prevFormData,
        [name]: value,
  ...
  return (
    ...
    <MDBContainer className="p-3 my-5 d-flex flex-column w-50">
      ...
      <MDBTabsContent>
        ...
        <MDBRow>
                <MDBCol col='6'>
                    <MDBInput wrapperClass='mb-4' label='(ex. 980101)' id='form11' type='number'
                    value={formData.m_jumin1} onChange={handleChange} onInput={(e) => {e.target.value = e.target.value.slice(0, 6);}}name="m_jumin1"/>
                </MDBCol>
